...
import my_filters
import currencies
from bot_commands import default_commands
...
def no_arg_to_set_my_currency(message: types.Message):
    bot.send_message(
        chat_id=message.chat.id,
        text=messages.set_my_currency_help_message,
        parse_mode="HTML",
    )
...
def set_selected_currency(
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=currency_success_message.format(currency=formatting.hcode(currency)),
        parse_mode="HTML",
    )
...
def set_default_currency(message: types.Message):
    set_selected_currency(
        message,
        data_key=default_currency_key,
        currency_success_message=messages.set_my_currency_success_message,
    )
...
def set_local_currency(message: types.Message):
    ...
    set_selected_currency(
        message,
        data_key=local_currency_key,
        currency_success_message=messages.set_local_currency_success_message,
    )
...
def query_is_only_digits(query: types.InlineQuery):
    if query and query.query:
        return query.query.isdigit()
    return False


@bot.inline_handler(func=query_is_only_digits)
def handle_digit_query(query: types.InlineQuery):
    ...
    currencies_ratios = currencies.get_currencies_ratios(
    ...
    results = []
    for currency_rate, currency_name in zip(currencies_ratios, target_currencies):
        total = currency_rate * amount
        total_str = f"{total:,.2f}"
        to_curr = currency_name.upper()
        content = types.InputTextMessageContent(
            message_text=formatting.format_text(
                f"{formatting.hcode(str(amount))} {from_curr} в {to_curr}:",
                formatting.hcode(total_str),
            ),
            parse_mode="HTML",
        )
        result_article = types.InlineQueryResultArticle(
            id=f"{from_curr}-{to_curr}-{amount}",
            title=f"{total_str} {to_curr}",
            description=f"{amount} {from_curr} = {total_str} {to_curr}",
